"""
Experiment 3: Minimal RMSNorm Comparison Trainer
Simple trainer to compare baseline vs DeepSeek RMSNorm with minimal code changes
"""
...
from experiments.exp3.exp3_baseline_model import BaselineMoEModel
from experiments.exp3.exp3_deepseek_model import DeepSeekMoEModel
...
class Experiment3Trainer:
    ...
    def run_experiment(self) -> Dict[str, Any]:
        ...
        print(f"\n🚀 Starting Experiment 3: RMSNorm Comparison")
        print(f"📋 Comparing: Baseline RMSNorm vs DeepSeek RMSNorm")
        ...
        deepseek_model = DeepSeekMoEModel(self.base_config)
        ...
        results['baseline'] = self.train_model(baseline_model, 'Baseline RMSNorm')
        
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        
        results['deepseek'] = self.train_model(deepseek_model, 'DeepSeek RMSNorm')
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        results_file = self.output_dir / "exp3_rmsnorm_comparison.json"
    ...
    def _print_comparison(self, results: Dict[str, Any]):
        ...
        print(f"\n{'='*80}")
        print(f"📊 EXPERIMENT 3 COMPARISON: RMSNorm Comparison")
        print(f"{'='*80}")
        ...
        if len(results) >= 2:
            ...
            if deepseek_loss < baseline_loss:
                ...
                print(f"🏆 DeepSeek RMSNorm is better by {improvement:.2f}%")
            else:
                improvement = (deepseek_loss - baseline_loss) / deepseek_loss * 100
                print(f"🏆 Baseline RMSNorm is better by {improvement:.2f}%")
    ...
    def _create_loss_vs_time_plot(self):
        ...
        plt.title('Validation Loss vs Time - Experiment 3: RMSNorm Comparison', fontsize=14)
...
def main():
    ...
    print(f"🚀 Experiment 3 Configuration (5x Longer Training):")
    ...
    # Create trainer
    trainer = Experiment3Trainer(base_config)
