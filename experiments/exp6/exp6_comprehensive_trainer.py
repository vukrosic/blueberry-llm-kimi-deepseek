"""
Experiment 6: Comprehensive Exhaustive Ablation Study Trainer
Testing ALL 27 possible combinations of DeepSeek components with 20-step tests
"""
...
from experiments.exp6.exp6_comprehensive_ablation_models import (
    create_comprehensive_ablation_model, 
    COMPREHENSIVE_ABLATION_MODELS,
    print_comprehensive_ablation_summary
)


class ComprehensiveExperiment6Trainer:
    """Comprehensive exhaustive ablation study trainer for all 27 model configurations"""
    
    def __init__(self, base_config: MoEModelConfig, output_dir: str = "experiments/exp6/exp6_comprehensive_results"):
    ...
    def run_comprehensive_test(self, test_steps: int = 20) -> Dict[str, Any]:
        """Run comprehensive 20-step test on all 27 models"""
        print(f"\n🚀 Starting Comprehensive Experiment 6: Exhaustive Ablation Study")
        print(f"📋 Testing {len(COMPREHENSIVE_ABLATION_MODELS)} models with {test_steps} steps each")
        ...
        for i, model_name in enumerate(COMPREHENSIVE_ABLATION_MODELS.keys(), 1):
            try:
                print(f"\n🧪 [{i}/{len(COMPREHENSIVE_ABLATION_MODELS)}] Testing {model_name}...")
                ...
                # Create model
                model = create_comprehensive_ablation_model(model_name, self.base_config)
        ...
        # Save results
        self._save_results(results, "comprehensive")
        
        # Print comprehensive comparison
        self._print_comprehensive_comparison(results)
        
        # Create comprehensive loss vs time plot
    ...
    def _save_results(self, results: Dict[str, Any], mode: str):
        ...
        results_file = self.output_dir / f"exp6_comprehensive_{mode}_results.json"
    ...
    def _print_comprehensive_comparison(self, results: Dict[str, Any]):
        """Print comprehensive comparison of all 27 models"""
        print(f"\n{'='*120}")
        print(f"📊 COMPREHENSIVE EXPERIMENT 6 COMPARISON: All {len(COMPREHENSIVE_ABLATION_MODELS)} Models")
        print(f"{'='*120}")
        ...
        print(f"{'Rank':<4} {'Model':<25} {'Val Loss':<10} {'Val Acc':<10} {'Val Perp':<10} {'Time (min)':<12} {'Params (M)':<12} {'Category':<20}")
        print(f"{'-'*120}")
        ...
        for rank, (name, result) in enumerate(sorted_results, 1):
            ...
            print(f"{rank:<4} {name:<25} {val_loss:<10.4f} {val_acc:<10.4f} {val_perp:<10.2f} {time_min:<12.2f} {params_m:<12.2f} {category:<20}")
    ...
    def _get_model_category(self, model_name: str) -> str:
        ...
        if model_name == "baseline":
            return "Baseline"
        elif model_name in ["rmsnorm", "mlp", "moe", "attention"]:
            return "Single Component"
        elif any(x in model_name for x in ["_2layers", "_4layers", "_128d", "_512d", "_4experts", "_16experts", "_top1", "_top4"]):
            return "Architecture Variant"
        elif any(x in model_name for x in ["_no_rope", "_no_bias", "_linear_rope"]):
            return "Attention Variant"
        elif model_name == "all_components":
    ...
    def _create_comprehensive_plot(self):
        ...
        colors = {
            ...
            'Multi Component': 'green', 
            'All Components': 'red',
            'Architecture Variant': 'orange',
            'Attention Variant': 'purple'
        }
        ...
        plt.title(f'Comprehensive Experiment 6: All {len(COMPREHENSIVE_ABLATION_MODELS)} Ablation Models - Validation Loss vs Time', fontsize=14)
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=8)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        
        # Save plot
        plot_file = self.output_dir / "exp6_comprehensive_loss_vs_time_comparison.png"
...
def main():
    """Main function to run Comprehensive Experiment 6"""
    # Check system
    ...
    # Create configuration for comprehensive testing (20 steps each)
    base_config = MoEModelConfig(
        max_steps=20,  # 20 steps per model for comprehensive testing
    ...
    print(f"🚀 Comprehensive Experiment 6 Configuration:")
    ...
    print(f"   Models: {len(COMPREHENSIVE_ABLATION_MODELS)} comprehensive ablation configurations")
    print(f"   Expected Total Time: ~{len(COMPREHENSIVE_ABLATION_MODELS) * 2} minutes (comprehensive testing)")
    
    # Create trainer
    trainer = ComprehensiveExperiment6Trainer(base_config)
    
    # Run comprehensive ablation study with 20 steps per model
    print(f"\n🧪 Running comprehensive ablation study (20 steps per model)...")
    
    # Run comprehensive experiment
    results = trainer.run_comprehensive_test(test_steps=20)
    
    print(f"\n✅ Comprehensive Experiment 6 completed!")
