"""
Experiment 6: DeepseekV3FlashAttention2 vs Baseline Attention Comparison Trainer
Fair comparison of attention implementations with same model size
"""
...
from experiments.exp6.exp6_baseline_attention_model import BaselineAttentionModel
from experiments.exp6.exp6_flash_attention_model import FlashAttentionModel


class Experiment6Trainer:
    """Trainer for FlashAttention comparison experiment"""
    ...
    def run_experiment(self) -> Dict[str, Any]:
        """Run the full experiment comparing both attention implementations"""
        print(f"\n🚀 Starting Experiment 6: Attention Implementation Comparison")
        print(f"📋 Comparing: Baseline Attention vs DeepseekV3Attention")
        
        # Set seed for reproducibility
        set_seed(42)
        
        # Create models
        baseline_model = BaselineAttentionModel(self.base_config)
        flash_model = FlashAttentionModel(self.base_config)
        ...
        results['baseline'] = self.train_model(baseline_model, 'Baseline Attention')
        
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        
        results['deepseek'] = self.train_model(flash_model, 'DeepseekV3Attention')
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        results_file = self.output_dir / "exp6_attention_comparison.json"
    ...
    def _print_comparison(self, results: Dict[str, Any]):
        """Print comparison of both attention implementations"""
        print(f"\n{'='*80}")
        print(f"📊 EXPERIMENT 6 COMPARISON: Attention Implementation Comparison")
        print(f"{'='*80}")
        
        print(f"{'Model':<25} {'Val Loss':<10} {'Val Acc':<10} {'Val Perp':<10} {'Time (min)':<12} {'Params (M)':<12}")
        print(f"{'-'*80}")
        ...
        for name, result in results.items():
            ...
            print(f"{name:<25} {val_loss:<10.4f} {val_acc:<10.4f} {val_perp:<10.2f} {time_min:<12.2f} {params_m:<12.2f}")
        ...
        if len(results) >= 2:
            ...
            flash_loss = results['flash']['val_loss']
            ...
            if deepseek_loss < baseline_loss:
                ...
                print(f"🏆 DeepseekV3Attention is better by {improvement:.2f}%")
            else:
                improvement = (deepseek_loss - baseline_loss) / deepseek_loss * 100
                print(f"🏆 Baseline Attention is better by {improvement:.2f}%")
    ...
    def _create_loss_vs_time_plot(self):
        ...
        plt.title('Validation Loss vs Time - Experiment 6: Attention Implementation Comparison', fontsize=14)
