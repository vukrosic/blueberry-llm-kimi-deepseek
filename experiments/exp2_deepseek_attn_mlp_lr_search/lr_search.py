...
Runs learning rate search for DeepSeek Attention + MLP model
"""
...
class LearningRateSearchTrainer:
    """Learning rate search trainer for DeepSeek Attention + MLP model"""
    ...
    def train_with_lr(self, learning_rate: float, max_steps: int = 1000, eval_every: int = 100) -> Dict[str, Any]:
        ...
        # Create model
        model = AttentionMLP_512dModel(self.config)
...
def run_lr_search():
    """Run learning rate search for DeepSeek Attention + MLP model"""
    print("🚀 Learning Rate Search for DeepSeek Attention + MLP")
    print("=" * 60)
    ...
    config = MoEModelConfig(
        ...
        d_model=512,
        n_heads=8,
        n_layers=6,       # Reduced layers for memory
        d_ff=2048,
        num_experts=8,
        expert_top_k=2,
    )
    ...
    print(f"   Model: DeepSeek Attention + MLP 512d")
