"""
Training script for Experiment 1: DeepSeek Attention Integration

This script trains and compares different attention configurations:
1. Baseline (standard attention)
2. DeepSeek with LoRA projections
3. DeepSeek with Flash Attention
4. DeepSeek with RoPE scaling
5. Full DeepSeek features

The goal is to measure the impact of each DeepSeek feature on model performance.
"""
...
from .exp1_config import get_experiment_configs
from .exp1_deepseek_attention import DeepSeekMoEModel
from models.moe_llm import MoEMinimalLLM


class Experiment1Trainer:
    """Trainer for Experiment 1: DeepSeek Attention Integration"""
    
    def __init__(self, base_config: MoEModelConfig, output_dir: str = "experiments/exp1_results"):
    ...
    def create_model(self, config: MoEModelConfig):
        """Create model based on configuration"""
        if hasattr(config, 'experiment_name') and 'deepseek' in config.experiment_name:
            return DeepSeekMoEModel(config)
        else:
            return MoEMinimalLLM(config)
    
    def train_configuration(self, config: MoEModelConfig) -> Dict[str, Any]:
        ...
        final_metrics['experiment_name'] = config.experiment_name
    ...
    def _print_config_details(self, config: MoEModelConfig):
        ...
        if hasattr(config, 'q_lora_rank') and config.q_lora_rank is not None:
            print(f"   DeepSeek Q LoRA rank: {config.q_lora_rank}")
        if hasattr(config, 'kv_lora_rank') and config.kv_lora_rank is not None:
            print(f"   DeepSeek KV LoRA rank: {config.kv_lora_rank}")
    ...
    def run_experiment(self, configs_to_run: List[str] = None) -> Dict[str, Any]:
        ...
        print(f"\n🚀 Starting Experiment 1: DeepSeek Attention Integration")
        ...
        for name, config in configs_to_run.items():
            try:
                results[name] = self.train_configuration(config)
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        results_file = self.output_dir / "experiment1_results.json"
    ...
    def _print_comparison(self, results: Dict[str, Any]):
        ...
        print(f"\n{'='*80}")
        print(f"📊 EXPERIMENT 1 COMPARISON: DeepSeek Attention Integration")
        print(f"{'='*80}")
        
        # Create comparison table
        print(f"{'Configuration':<20} {'Val Loss':<10} {'Val Acc':<10} {'Val Perp':<10} {'Time (min)':<12}")
        print(f"{'-'*80}")
        ...
        for name, result in results.items():
            if "error" not in result:
                ...
                print(f"{name:<20} {val_loss:<10.4f} {val_acc:<10.4f} {val_perp:<10.2f} {time_min:<12.1f}")
            else:
                print(f"{name:<20} {'ERROR':<10} {'ERROR':<10} {'ERROR':<10} {'ERROR':<12}")
...
def main():
    ...
    # Create trainer
    trainer = Experiment1Trainer(base_config)
