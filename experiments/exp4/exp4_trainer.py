"""
Experiment 4: DeepseekV3MLP vs Baseline MLP Comparison Trainer
Simple trainer to compare baseline MLP vs DeepseekV3MLP with minimal code changes
"""
...
from experiments.exp4.exp4_baseline_model import BaselineMLPMoEModel
from experiments.exp4.exp4_deepseek_mlp_model import DeepseekV3MLPMoEModel


class Experiment4Trainer:
    """Minimal trainer for MLP comparison"""
    
    def __init__(self, base_config: MoEModelConfig, output_dir: str = "experiments/exp4/exp4_results"):
    ...
    def train_model(self, model, model_name: str) -> Dict[str, Any]:
        """Train a single model with minimal metrics"""
        print(f"\n{'='*60}")
    ...
    def run_experiment(self) -> Dict[str, Any]:
        ...
        print(f"\n🚀 Starting Experiment 4: MLP Comparison")
        print(f"📋 Comparing: Baseline MLP vs DeepseekV3MLP")
        
        # Set seed for reproducibility
        set_seed(42)
        
        # Create models
        baseline_model = BaselineMLPMoEModel(self.base_config)
        deepseek_model = DeepseekV3MLPMoEModel(self.base_config)
        ...
        results['baseline'] = self.train_model(baseline_model, 'Baseline MLP')
        
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        
        results['deepseek'] = self.train_model(deepseek_model, 'DeepseekV3MLP')
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        results_file = self.output_dir / "exp4_mlp_comparison.json"
    ...
    def _print_comparison(self, results: Dict[str, Any]):
        ...
        print(f"\n{'='*80}")
        print(f"📊 EXPERIMENT 4 COMPARISON: MLP Comparison")
        print(f"{'='*80}")
        ...
        if len(results) >= 2:
            ...
            if deepseek_loss < baseline_loss:
                ...
                print(f"🏆 DeepseekV3MLP is better by {improvement:.2f}%")
            else:
                improvement = (deepseek_loss - baseline_loss) / deepseek_loss * 100
                print(f"🏆 Baseline MLP is better by {improvement:.2f}%")
    ...
    def _create_loss_vs_time_plot(self):
        ...
        plt.title('Validation Loss vs Time - Experiment 4: MLP Comparison', fontsize=14)
...
def main():
    ...
    # Create minimal configuration with 5x longer training
    base_config = MoEModelConfig(
        max_steps=1000,  # 5x longer training
        batch_size=16,  # Small batch size
        max_tokens=100000,  # Small dataset
        eval_every=100,  # Evaluate every 100 steps
        num_documents=1000,  # Small dataset
        max_seq_len=256,  # Short sequences
        d_model=256,  # Small model
        n_heads=4,    # Small model
        n_layers=3,   # Small model
        d_ff=1024,    # Small model
    )
    
    print(f"🚀 Experiment 4 Configuration (5x Longer Training):")
    print(f"   Steps: {base_config.max_steps} (5x longer)")
    ...
    # Create trainer
    trainer = Experiment4Trainer(base_config)
